...
class NanoBananaProvider(BaseImageGenProvider, MLflowTracingMixin):
    ...
    def __init__(self, model_config):
        ...
        # 模型名称
        self.model = getattr(model_config, 'model_name', 'gemini-3-pro-image-preview')
        
        logger.info(
            "NanoBananaProvider初始化成功",
            operation="nano_banana_init_success",
            model=self.model,
    ...
    async def generate_image(
        ...
        **kwargs
        ...
        try:
            ...
            logger.info(
                "调用Gemini API生成图片",
                operation="nano_banana_generate_start",
                model=self.model,
            ...
            logger.debug(
                "Gemini API响应完成",
                operation="nano_banana_api_response",
            ...
            for i, part in enumerate(response.parts):
                ...
                if hasattr(part, 'inline_data') and part.inline_data is not None:
                    try:
                        ...
                        logger.info(
                            "成功从inline_data提取图片",
                            operation="nano_banana_image_extracted",
                ...
                try:
                    ...
                    if image:
                        logger.info(
                            "成功通过as_image()提取图片",
                            operation="nano_banana_image_extracted_as_image",
                            image_size=image.size
                        )
            ...
            logger.error(
                error_msg,
                operation="nano_banana_generation_failed",
                exception=e
            )
            return ImageGenerationResult(
                success=False,
