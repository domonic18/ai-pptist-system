# AI PPT 图片编辑功能架构设计

## 1. 背景与目标

### 1.1 背景

当前 AI PPTist System 已实现基于文生图的 PPT 自动生成功能（香蕉生成），该功能通过以下流程生成 PPT：

1. 用户输入主题，AI 生成 PPT 大纲
2. 用户确认大纲后，后端分析内容并调用文生图模型生成 PPT 图片
3. 前端将生成的图片显示在编辑器中

**核心问题**：由于生成的 PPT 是图片格式，导致其中的文字内容不可编辑，限制了用户的后续修改能力。

### 1.2 目标

在现有系统中增加 **PPT 图片解析功能**，实现：

1. 点击前端工具条按钮触发图片解析
2. 后端解析图片中的文字，识别文字内容及其在图片中的区域位置
3. 前端在原始图片上叠加显示可编辑的文字框架，用户可以直接修改文字

**说明**：用户编辑后的文字仅存在于前端内存中，用于演示和查看。如需保存修改后的 PPT，属于未来扩展功能。

---

## 2. 核心功能概述

### 2.1 功能定位

本功能专注于 **文字识别与区域定位**，不涉及 PPT 文件保存或持久化。

**核心能力**：
- 文字检测：识别图片中所有文字区域的位置（边界框）
- 文字识别：识别每个区域内的文字内容
- 区域返回：返回文字内容及其对应的坐标位置
- 叠加渲染：前端在图片上叠加显示可编辑的文字框
- **前端编辑**：编辑、拖拽移动、调整大小（仅内存，不持久化）

**不在本次范围**：
- ❌ 保存修改后的文字到后端
- ❌ 重新生成 PPT 文件
- ❌ 导出为可编辑的 PPTX

### 2.2 用户交互流程

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│  用户查看生成的  │  →  │  点击"解析图片"  │  →  │  显示解析进度   │
│  PPT图片        │     │  按钮            │     │                 │
└─────────────────┘     └─────────────────┘     └─────────────────┘
                                                          │
                                                          ▼
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│  编辑文字内容    │  ←  │  在图片上叠加    │  ←  │  解析完成       │
│  (仅内存，不保存)│     │  显示可编辑框    │     │                 │
└─────────────────┘     └─────────────────┘     └─────────────────┘
```

---

## 3. 技术选型分析

### 3.1 MVP 阶段 - 最佳演示效果选型

**项目现状与需求**：
- MVP 原型需要良好的演示效果
- 无高性能 GPU 机器
- 无法进行复杂的环境部署
- 需要快速集成

#### OCR 方案对比（按综合效果排序）

| 方案 | 类型 | 识别准确率 | 部署复杂度 | 是否需要GPU | 成本 | 演示推荐度 |
|------|------|-----------|-----------|-----------|------|-----------|
| **腾讯云OCR** | API | ⭐⭐⭐⭐⭐ 95%+ | ⭐ 零配置 | ❌ 不需要 | 免费+付费 | ⭐⭐⭐⭐⭐ |
| **阿里云OCR** | API | ⭐⭐⭐⭐⭐ 95%+ | ⭐ 零配置 | ❌ 不需要 | 免费+付费 | ⭐⭐⭐⭐⭐ |
| **百度OCR** | API | ⭐⭐⭐⭐ 90%+ | ⭐ 零配置 | ❌ 不需要 | 免费+付费 | ⭐⭐⭐⭐ |
| **EasyOCR** | 开源 | ⭐⭐⭐ 85-90% | ⭐⭐ 需安装 | ❌ CPU可用 | 免费 | ⭐⭐⭐ |
| **PaddleOCR** | 开源 | ⭐⭐⭐⭐ 90%+ | ⭐⭐⭐ 复杂 | ✅ GPU最佳 | 免费 | ⭐⭐ |

#### 3.1.1 MVP 首选方案：腾讯云 OCR API

**选择理由**：

1. **识别效果最好**
   - 准确率：95%+（行业领先）
   - 专为PPT场景优化
   - 支持表格、公式等复杂内容

2. **零配置部署**
   - 无需安装任何OCR库
   - 只需配置API密钥

3. **无需GPU**
   - 云端处理，本地无压力
   - 任何机器都能运行

4. **免费额度充足**
   - 腾讯云OCR每月免费额度：
     - 通用印刷体识别：1000次/月
     - 增强印刷体识别：500次/月
   - MVP 演示完全够用


### 3.2 技术选型总结

```
MVP 阶段（当前）          生产环境（未来）
    │                          │
    ▼                          ▼
┌─────────────┐          ┌─────────────┐
│ 腾讯云OCR   │  ──────> │ 自建OCR集群 │
│   API       │  成本优化 │ 或混合云    │
└─────────────┘          └─────────────┘
   5分钟配置                降低长期成本
   最佳演示效果
```

---

## 4. 整体架构设计

### 4.1 系统架构图

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                              前端层 (Vue 3 + TypeScript)                     │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                    编辑器视图组件 (SlideEditor.vue)                   │   │
│  │                                                                     │   │
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │                    画布容器 (Canvas)                        │    │   │
│  │  │                                                             │    │   │
│  │  │  ┌─────────────────────────────────────────────────────┐   │    │   │
│  │  │  │              背景图片层 (原始PPT图片)                │   │    │   │
│  │  │  │                                                     │   │    │   │
│  │  │  └─────────────────────────────────────────────────────┘   │    │   │
│  │  │                           │                                │    │   │
│  │  │                           ▼                                │    │   │
│  │  │  ┌─────────────────────────────────────────────────────┐   │    │   │
│  │  │  │           叠加文字框层 (可编辑元素)                  │   │    │   │
│  │  │  │  ┌──────────┐  ┌──────────┐  ┌──────────┐          │   │    │   │
│  │  │  │  │ 文字框1  │  │ 文字框2  │  │ 文字框3  │  ...     │   │    │   │
│  │  │  │  │ (可编辑) │  │ (可编辑) │  │ (可编辑) │          │   │    │   │
│  │  │  │  └──────────┘  └──────────┘  └──────────┘          │   │    │   │
│  │  │  └─────────────────────────────────────────────────────┘   │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  │                                                                     │   │
│  │  ┌─────────────────────────────────────────────────────────────┐    │   │
│  │  │                    工具条 (Toolbar)                         │    │   │
│  │  │  [解析图片] [重新解析]                                      │    │   │
│  │  └─────────────────────────────────────────────────────────────┘    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │                    PPT解析服务 (imageParsingService.ts)             │   │
│  │  - parseSlideImage()     - getParsingStatus()                      │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        │ HTTP/WebSocket
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                            API网关层 (FastAPI)                               │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌──────────────────────────────────────────────────────────────────────┐  │
│  │                   图片解析端点 (image_parsing.py)                     │  │
│  │                                                                       │  │
│  │  POST /api/v1/image_parsing/parse       解析图片中的文字             │  │
│  │  GET  /api/v1/image_parsing/status/{task_id}  查询解析状态           │  │
│  └──────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
                                        │
                                        ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                            服务层 (Services)                                 │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌─────────────────────────────┐    ┌─────────────────────────────────┐   │
│  │   图片解析服务               │    │   任务管理服务                   │   │
│   │   (ImageParsingService)    │    │   (ParsingTaskManager)          │   │
│   │                            │    │                                 │   │
│  │ - parse_image()             │    │ - create_task()                 │   │
│  │ - process_ocr_result()      │    │ - update_status()               │   │
│  │ - format_text_regions()     │    │ - save_result()                 │   │
│  └─────────────────────────────┘    └─────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
                    │                               │
                    ▼                               ▼
┌───────────────────────────────┐   ┌───────────────────────────────────────┐
│      OCR引擎                  │   │         存储层                         │
├───────────────────────────────┤   ├───────────────────────────────────────┤
│                               │   │                                       │
│  ┌─────────────────────────┐  │   │  ┌─────────────┐  ┌───────────────┐  │
│  │  腾讯云OCR API          │  │   │  │ PostgreSQL  │  │    Redis      │  │
│  │  - 文字检测与识别        │  │   │  │             │  │               │  │
│  │  - 高精度识别(95%+)     │  │   │  │ - 解析任务  │  │ - 任务状态    │  │
│  │  - 零配置部署            │  │   │  │ - 文字区域  │  │ - 解析结果    │  │
│  └─────────────────────────┘  │   │  │             │  │ - 缓存数据    │  │
│                               │   │  └─────────────┘  └───────────────┘  │
│                               │   │                                       │
│                               │   │  ┌─────────────────────────────────┐  │
│                               │   │  │  腾讯云COS                       │  │
│                               │   │  │  - 原始PPT图片                   │  │
│                               │   │  └─────────────────────────────────┘  │
│                               │   │                                       │
└───────────────────────────────┘   └───────────────────────────────────────┘
```

### 4.2 分层架构说明

#### 4.2.1 前端层

**重要原则**：复用现有成熟组件，避免重复造轮子。

**实际目录结构**（基于现有代码）：

```
frontend/src/
├── views/
│   └── Editor/
│       ├── index.vue                          # ✅ 现有 - 主编辑器布局
│       ├── Canvas/
│       │   ├── index.vue                      # ✅ 现有 - 画布容器（已有元素渲染）
│       │   ├── EditableElement.vue            # ✅ 现有 - 元素编辑包装器
│       │   ├── ViewportBackground.vue         # ✅ 现有 - 画布背景
│       │   ├── TextOverlayLayer.vue           # ⭐ 新增 - OCR文字叠加层
│       │   └── Operate/                       # ✅ 现有 - 元素操作（缩放、旋转等）
│       ├── CanvasTool/
│       │   └── index.vue                      # ✅ 现有 - 画布工具条（插入元素按钮）
│       ├── Toolbar/
│       │   ├── index.vue                      # ✅ 现有 - 工具栏（需添加解析按钮）
│       │   └── ElementStylePanel/             # ✅ 现有 - 元素样式面板
│       └── Thumbnails/
│           └── index.vue                      # ✅ 现有 - 幻灯片缩略图
├── services/
│   ├── index.ts                               # ✅ 现有 - 主要API服务
│   ├── image.ts                               # ✅ 现有 - 图片相关服务
│   └── imageParsingService.ts                 # ⭐ 新增 - 图片解析服务
├── configs/
│   └── api.ts                                 # 🔧 修改 - 添加IMAGE_PARSING端点
├── types/
│   ├── slides.ts                              # ✅ 现有 - 幻灯片类型定义
│   └── imageParsing.ts                        # ⭐ 新增 - 图片解析类型定义
└── store/
    ├── main.ts                                # 🔧 修改 - 添加解析状态管理
    └── slides.ts                              # ✅ 现有 - 幻灯片状态
```

**图例说明**：
- ✅ 现有 - 已有代码，直接复用
- 🔧 修改 - 需要小幅修改
- ⭐ 新增 - 需要新建

**组件复用与变更说明**：

| 组件/文件 | 状态 | 说明 | 变更内容 |
|----------|------|------|----------|
| `Editor/index.vue` | ✅ 现有 | 主编辑器布局 | 无需修改 |
| `Canvas/index.vue` | 🔧 修改 | 画布容器 | 添加TextOverlayLayer渲染 |
| `Canvas/EditableElement.vue` | ✅ 现有 | 元素编辑器 | 直接复用，已有完整编辑功能 |
| `Canvas/ViewportBackground.vue` | ✅ 现有 | 背景层 | 直接复用 |
| `Canvas/TextOverlayLayer.vue` | ⭐ 新增 | 文字叠加层 | 新建，渲染OCR识别的文字框 |
| `CanvasTool/index.vue` | ✅ 现有 | 工具条 | 无需修改 |
| `Toolbar/index.vue` | 🔧 修改 | 右侧工具栏 | 添加"解析图片"按钮 |
| `services/imageParsingService.ts` | ⭐ 新增 | 解析服务 | 新建，封装OCR API调用 |
| `configs/api.ts` | 🔧 修改 | API配置 | 添加IMAGE_PARSING端点 |
| `types/imageParsing.ts` | ⭐ 新增 | 类型定义 | 新建，定义解析相关类型 |
| `store/main.ts` | 🔧 修改 | 状态管理 | 添加解析状态字段 |

**核心实现要点**：

1. **复用 `EditableElement.vue`**：
   - 现有组件已实现完整的文本编辑功能
   - 支持编辑、拖拽、缩放、旋转
   - 只需传入OCR识别的TextRegion数据即可

2. **复用 `Canvas/index.vue`**：
   - 现有画布已有元素渲染系统
   - 只需添加 `TextOverlayLayer` 组件渲染OCR结果

3. **最小化新增代码**：
   - `TextOverlayLayer.vue`：遍历OCR结果，渲染文字框
   - `imageParsingService.ts`：封装API调用
   - 工具栏按钮：触发解析操作

**服务封装** (`imageParsingService.ts` - 新增)：

```typescript
// frontend/src/services/imageParsingService.ts
import axios from 'axios'
import { API_CONFIG } from '@/configs/api'
import type { ImageParseResult, ParseTaskResponse, TextRegion } from '@/types/imageParsing'

export const imageParsingService = {
  /**
   * 解析图片中的文字
   */
  async parseSlideImage(
    slideId: string,
    imageUrl: string
  ): Promise<ParseTaskResponse> {
    const response = await axios.post(API_CONFIG.IMAGE_PARSING.PARSE, {
      slide_id: slideId,
      image_url: imageUrl
    })
    return response.data.data
  },

  /**
   * 查询解析状态
   */
  async getParsingStatus(taskId: string): Promise<ImageParseResult> {
    const response = await axios.get(
      API_CONFIG.IMAGE_PARSING.STATUS(taskId)
    )
    return response.data.data
  }
}
```

**类型定义** (`types/imageParsing.ts` - 新增)：

```typescript
// frontend/src/types/imageParsing.ts
export interface ParseTaskResponse {
  task_id: string
  status: 'pending' | 'processing' | 'completed' | 'failed'
  estimated_time: number
}

export interface BoundingBox {
  x: number
  y: number
  width: number
  height: number
}

export interface TextRegion {
  id: string
  text: string
  bbox: BoundingBox
  confidence: number
  font?: {
    size: number
    family: string
    weight: 'normal' | 'bold'
  }
  // 前端编辑时临时使用（不持久化）
  _isModified?: boolean
  _originalText?: string
}

export interface ImageParseResult {
  task_id: string
  slide_id: string
  image_url: string
  status: 'pending' | 'processing' | 'completed' | 'failed'
  progress: number
  text_regions: TextRegion[]
  metadata: {
    parse_time: number
    ocr_engine: string
    text_count: number
    created_at: string
    completed_at?: string
  }
}
```

#### 4.2.2 API网关层

**端点定义** (`image_parsing.py`):

| 端点 | 方法 | 功能 |
|------|------|------|
| `/api/v1/image_parsing/parse` | POST | 解析图片中的文字 |
| `/api/v1/image_parsing/status/{task_id}` | GET | 查询解析状态 |

#### 4.2.3 服务层

1. **ImageParsingService** - 核心解析服务
   - 调用OCR引擎进行文字识别
   - 处理OCR结果并格式化
   - 构建文字区域数据结构

2. **ParsingTaskManager** - 任务管理服务
   - 创建和跟踪解析任务
   - 管理任务状态和进度
   - 保存和缓存解析结果

#### 4.2.4 存储层

- **PostgreSQL**: 持久化存储解析任务和解析结果
- **Redis**: 缓存解析状态和结果（提升查询性能）
- **腾讯云COS**: 存储原始PPT图片

**注意**：用户在前端编辑的文字内容**不持久化**，仅存在于前端内存中。

---

## 5. 核心实现流程

### 5.1 图片解析流程

```
┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐
│ 用户点击  │ → │ 前端调用 │ → │ 后端接收 │ → │ 创建任务 │ → │ 下载图片 │
│ 解析按钮  │   │ API接口  │   │ 请求     │   │ 返回ID   │   │ 到本地   │
└────┬─────┘   └────┬─────┘   └────┬─────┘   └────┬─────┘   └────┬─────┘
     │              │              │              │              │
     │              │              ▼              │              │
     │              │         ┌─────────┐        │              │
     │              │         │ 保存任务 │        │              │
     │              │         │ 到数据库 │        │              │
     │              │         └────┬────┘        │              │
     │              │              │              │              │
     │              │              ▼              │              ▼
     │              │         ┌─────────┐        │    ┌─────────────┐
     │              │         │ 提交异步 │        │    │ 腾讯云OCR   │
     │              │         │ Celery   │ ──────┼──→ │ API调用     │
     │              │         │ 任务     │        │    │ 文字检测    │
     │              │         └────┬────┘        │    └──────┬──────┘
     │              │              │              │           │
     │              │              │              │           ▼
     │              │              │              │    ┌─────────────┐
     │              │              │              │    │ 格式化结果  │
     │              │              │              │    │ 构建文字区域│
     │              │              │              │    └──────┬──────┘
     │              │              │              │           │
     │              │              ▼              ▼           ▼
     │              │         ┌──────────────────────────────┐
     │              │         │ 保存结果到Redis             │
     │              │         │ 更新任务状态为completed     │
     │              │         └──────────────┬───────────────┘
     │              │                        │
     │              ▼                        ▼
     │         ┌────────────────────────────────────┐
     └─────────│ 前端轮询获取解析结果               │
               │ 或WebSocket推送                   │
               └────────────┬───────────────────────┘
                            │
                            ▼
                  ┌─────────────────────┐
                  │ 在图片上叠加显示     │
                  │ 可编辑文字框         │
                  └─────────────────────┘
```

### 5.2 数据结构定义

#### 5.2.1 文字区域数据结构

```typescript
// 解析结果数据结构
interface ImageParseResult {
  taskId: string
  slideId: string
  imageUrl: string
  status: 'pending' | 'processing' | 'completed' | 'failed'
  progress: number  // 0-100
  textRegions: TextRegion[]
  metadata: ParseMetadata
}

// 文字区域
interface TextRegion {
  id: string                    // 唯一标识
  text: string                  // 识别的文字内容
  bbox: BoundingBox             // 边界框坐标
  confidence: number            // 识别置信度 0-1
  font?: FontInfo               // 推断的字体信息（可选）
}

// 边界框坐标
interface BoundingBox {
  x: number      // 左上角X坐标（像素）
  y: number      // 左上角Y坐标（像素）
  width: number  // 宽度（像素）
  height: number // 高度（像素）
  // 或者使用四角坐标
  points?: Point[]  // [[x1,y1], [x2,y2], [x3,y3], [x4,y4]]
}

interface Point {
  x: number
  y: number
}

// 字体信息（推断）
interface FontInfo {
  size: number           // 字体大小（估计值）
  family: string        // 字体族（默认Microsoft YaHei）
  weight: 'normal' | 'bold'  // 字重
  color?: string        // 字体颜色（如果可检测）
}

// 解析元数据
interface ParseMetadata {
  parseTime: number      // 解析耗时（毫秒）
  ocrEngine: string     // OCR引擎名称
  textCount: number     // 识别的文字数量
  createdAt: string     // 创建时间
  completedAt?: string  // 完成时间
}

// 修改后的文字区域
interface ModifiedTextRegion extends TextRegion {
  originalText: string  // 原始文字（用于对比）
  isModified: boolean   // 是否被修改过
  modifiedAt?: string   // 修改时间
}
```

#### 5.2.2 后端数据模型

```python
from pydantic import BaseModel
from typing import List, Optional
from datetime import datetime

class BoundingBox(BaseModel):
    """边界框坐标"""
    x: int
    y: int
    width: int
    height: int

class FontInfo(BaseModel):
    """字体信息"""
    size: int
    family: str = "Microsoft YaHei"
    weight: str = "normal"
    color: Optional[str] = None

class TextRegion(BaseModel):
    """文字区域"""
    id: str
    text: str
    bbox: BoundingBox
    confidence: float
    font: Optional[FontInfo] = None

class ParseMetadata(BaseModel):
    """解析元数据"""
    parse_time: int
    ocr_engine: str
    text_count: int
    created_at: datetime
    completed_at: Optional[datetime] = None

class ImageParseResult(BaseModel):
    """图片解析结果"""
    task_id: str
    slide_id: str
    image_url: str
    status: str
    progress: int
    text_regions: List[TextRegion]
    metadata: ParseMetadata

class ParseRequest(BaseModel):
    """解析请求"""
    slide_id: str
    image_url: str
    options: Optional[ParseOptions] = None

class ParseOptions(BaseModel):
    """解析选项（MVP阶段使用腾讯云OCR，预留扩展）"""
    # MVP阶段使用腾讯云OCR，无需传递额外参数
    # 未来可扩展：language, detect_tables 等选项
```

### 5.3 API接口定义

#### 5.3.1 解析图片

**请求**:

```http
POST /api/v1/image_parsing/parse
Content-Type: application/json
Authorization: Bearer <token>

{
  "slide_id": "banana_task_xxx_slide_0",
  "image_url": "https://xxx.cos.ap-beijing.myqcloud.com/ai-generated/ppt/xxx/slide_0.png"
}
```

**说明**：
- MVP阶段使用腾讯云OCR API，无需传递 `options` 参数
- 后端自动调用腾讯云OCR进行文字识别
- 未来扩展可支持 options 参数（如多语言、表格检测等）

**响应**:

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "code": 0,
  "message": "解析任务已创建",
  "data": {
    "task_id": "parse_20241224_123456_abc123",
    "status": "pending",
    "estimated_time": 5,
    "message": "任务已提交，正在处理中"
  },
  "timestamp": "2024-12-24T12:34:56Z",
  "request_id": "req_abc123"
}
```

#### 5.3.2 查询解析状态

**请求**:

```http
GET /api/v1/image_parsing/status/parse_20241224_123456_abc123
Authorization: Bearer <token>
```

**处理中响应**:

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "code": 0,
  "message": "正在处理",
  "data": {
    "task_id": "parse_20241224_123456_abc123",
    "status": "processing",
    "progress": 65,
    "current_step": "ocr_recognition",
    "message": "正在进行文字识别..."
  },
  "timestamp": "2024-12-24T12:35:00Z",
  "request_id": "req_def456"
}
```

**完成响应**:

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "code": 0,
  "message": "解析完成",
  "data": {
    "task_id": "parse_20241224_123456_abc123",
    "slide_id": "banana_task_xxx_slide_0",
    "image_url": "https://xxx.cos.ap-beijing.myqcloud.com/ai-generated/ppt/xxx/slide_0.png",
    "status": "completed",
    "progress": 100,
    "text_regions": [
      {
        "id": "region_001",
        "text": "人工智能技术发展现状",
        "bbox": {
          "x": 250,
          "y": 80,
          "width": 500,
          "height": 60
        },
        "confidence": 0.98,
        "font": {
          "size": 42,
          "family": "Microsoft YaHei",
          "weight": "bold"
        }
      },
      {
        "id": "region_002",
        "text": "人工智能技术正在快速发展，广泛应用于各个领域。",
        "bbox": {
          "x": 100,
          "y": 180,
          "width": 800,
          "height": 45
        },
        "confidence": 0.95,
        "font": {
          "size": 24,
          "family": "Microsoft YaHei",
          "weight": "normal"
        }
      },
      {
        "id": "region_003",
        "text": "应用领域包括：医疗、金融、教育、交通等。",
        "bbox": {
          "x": 100,
          "y": 250,
          "width": 800,
          "height": 45
        },
        "confidence": 0.96,
        "font": {
          "size": 24,
          "family": "Microsoft YaHei",
          "weight": "normal"
        }
      }
    ],
    "metadata": {
      "parse_time": 3200,
      "ocr_engine": "tencent_ocr",
      "text_count": 3,
      "created_at": "2024-12-24T12:34:56Z",
      "completed_at": "2024-12-24T12:35:00Z"
    }
  },
  "timestamp": "2024-12-24T12:35:01Z",
  "request_id": "req_xyz789"
}
```

### 5.4 前端渲染流程

#### 5.4.1 核心方案：复用现有文字编辑组件 + 遮罩

**设计原则**：最大化复用现有成熟组件，避免重复造轮子。

**方案概述**：
- 直接复用 `EditableElement.vue`（现有元素编辑组件）
- 使用遮罩层遮蔽原图片中的文字
- OCR 结果转换为 PPTTextElement 格式

**层级结构**：

```
┌─────────────────────────────────────────────────────────┐
│  Canvas (画布容器)                                        │
│  ┌───────────────────────────────────────────────────┐  │
│  │  ViewportBackground (原始PPT图片)                  │  │
│  │  人工智能技术发展 ← 原始不可编辑文字               │  │
│  └───────────────────────────────────────────────────┘  │
│  ┌───────────────────────────────────────────────────┐  │
│  │  TextOverlayLayer (OCR文字叠加层) ⭐ 新增          │  │
│  │  ┌─────────────────────────────────────────────┐  │  │
│  │  │  TextRegionWrapper (相对定位)               │  │  │
│  │  │  ┌───────────────────────────────────────┐  │  │  │
│  │  │  │  text-mask (遮罩层)                   │  │  │  │
│  │  │  │  background: rgba(255,255,255,0.95)   │  │  │  │
│  │  │  │  z-index: 1                           │  │  │  │
│  │  │  └───────────────────────────────────────┘  │  │  │
│  │  │  ┌───────────────────────────────────────┐  │  │  │
│  │  │  │  EditableElement (复用现有组件✅)     │  │  │  │
│  │  │  │  人工智能技术发展 ← 可编辑文字        │  │  │  │
│  │  │  │  z-index: 2                           │  │  │  │
│  │  │  └───────────────────────────────────────┘  │  │  │
│  │  └─────────────────────────────────────────────┘  │  │
│  └───────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
```

**核心实现代码**：

```vue
<!-- frontend/src/views/Editor/Canvas/TextOverlayLayer.vue -->
<template>
  <div
    v-if="showOverlay && textRegions.length > 0"
    class="text-overlay-layer"
  >
    <div
      v-for="region in textRegions"
      :key="region.id"
      class="text-region-wrapper"
      :style="wrapperStyle(region)"
    >
      <!-- 遮罩层：遮蔽原图片中的文字 -->
      <div class="text-mask"></div>

      <!-- 复用现有的文字编辑组件 -->
      <EditableElement
        :elementInfo="toTextElement(region)"
        :selectElement="handleSelect"
        :isMultiSelect="false"
      />
    </div>
  </div>
</template>

<script setup lang="ts">
import EditableElement from './EditableElement.vue'
import type { TextRegion } from '@/types/imageParsing'
import type { PPTTextElement } from '@/types/slides'

interface Props {
  textRegions: TextRegion[]
  showOverlay: boolean
}

const props = defineProps<Props>()

// 计算包装器样式（绝对定位）
const wrapperStyle = (region: TextRegion) => ({
  left: region.bbox.x + 'px',
  top: region.bbox.y + 'px',
  width: region.bbox.width + 'px',
  height: region.bbox.height + 'px',
})

/**
 * 将 OCR 识别的 TextRegion 转换为现有的 PPTTextElement 格式
 *
 * 转换映射：
 * - region.text → element.content
 * - region.bbox → element.{x, y, width, height}
 * - region.font → element.defaultFontSize/defaultFontName/etc
 */
const toTextElement = (region: TextRegion): PPTTextElement => {
  // 根据边界框高度推断字体大小（经验值：高度 * 0.7）
  const inferredFontSize = Math.round(region.bbox.height * 0.7)

  return {
    type: 'text',
    id: `ocr_text_${region.id}`,
    x: 0, // 相对定位，在 wrapper 内部为 0
    y: 0,
    width: region.bbox.width,
    height: region.bbox.height,
    content: region.text,
    defaultFontName: region.font?.family || 'Microsoft YaHei',
    defaultFontSize: Math.max(12, Math.min(72, inferredFontSize)),
    defaultColor: '#000000',
    defaultFontWeight: region.font?.weight === 'bold' ? 'bold' : 'normal',
    defaultAlign: 'center',
    defaultVerticalAlign: 'middle',
    fill: 'rgba(255, 255, 255, 0)', // 透明背景
  }
}

// 空的选中处理函数（EditableElement 要求传入）
const handleSelect = () => {}
</script>

<style scoped>
.text-overlay-layer {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  z-index: 100;
  pointer-events: none; /* 让未选中的区域点击穿透 */
}

.text-region-wrapper {
  position: absolute;
  pointer-events: auto; /* 文字区域可以交互 */
}

/* 遮罩层：遮蔽原图片中的文字 */
.text-mask {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background-color: white;
  opacity: 0.95;
  z-index: 1;
}
</style>
```

**数据转换示意**：

```typescript
// OCR 返回格式
TextRegion {
  id: "region_001",
  text: "人工智能技术发展现状",
  bbox: { x: 250, y: 80, width: 500, height: 60 },
  confidence: 0.98,
  font: { size: 42, family: "Microsoft YaHei", weight: "bold" }
}

         ↓ toTextElement() 转换

// 现有 PPT 元素格式
PPTTextElement {
  type: "text",
  id: "ocr_text_region_001",
  x: 0, y: 0,
  width: 500, height: 60,
  content: "人工智能技术发展现状",
  defaultFontSize: 42,
  defaultFontName: "Microsoft YaHei",
  defaultFontWeight: "bold",
  defaultAlign: "center",
  // ... 其他属性
}
```

**方案优势**：

| 优势 | 说明 |
|------|------|
| **零开发成本** | 直接复用 `EditableElement`，无需重新实现编辑功能 |
| **功能完整** | 自动获得编辑、拖拽、缩放、旋转、样式调整等所有功能 |
| **样式一致** | 与现有 PPT 编辑体验完全一致 |
| **维护简单** | 只需维护遮罩层和数据转换逻辑 |
| **扩展性强** | 未来系统升级 EditableElement，自动获得新功能 |

**Canvas 集成示例**：

```vue
<!-- frontend/src/views/Editor/Canvas/index.vue (部分修改) -->
<template>
  <div class="viewport">
    <!-- 现有元素 -->
    <EditableElement
      v-for="(element, index) in elementList"
      :key="element.id"
      :elementInfo="element"
      v-show="!hiddenElementIdList.includes(element.id)"
    />

    <!-- 新增：OCR 文字叠加层 -->
    <TextOverlayLayer
      v-if="ocrParseResult"
      :textRegions="ocrParseResult.text_regions"
      :showOverlay="showOCRTextOverlay"
    />
  </div>
</template>

<script setup lang="ts">
import TextOverlayLayer from './TextOverlayLayer.vue'
import { ref } from 'vue'
import { imageParsingService } from '@/services/imageParsingService'

const ocrParseResult = ref(null)
const showOCRTextOverlay = ref(false)

// 解析当前幻灯片图片
async function parseCurrentSlideImage() {
  const currentSlide = currentSlide.value
  if (!currentSlide) return

  // 提交解析任务
  const response = await imageParsingService.parseSlideImage(
    currentSlide.id,
    currentSlide.backgroundImage?.src || ''
  )

  // 轮询获取结果
  const result = await pollParsingStatus(response.task_id)
  ocrParseResult.value = result
  showOCRTextOverlay.value = true
}
</script>
```

#### 5.4.2 遮蔽原文字的原理

**为什么需要遮罩**：
- 原图片中的文字是"画"在图片里的，无法单独删除
- 如果不遮蔽，叠加的编辑文字会和原文字重叠显示
- 用白色遮罩层覆盖原文字，实现"视觉删除"效果

**遮罩层级关系**：

```
z-index: 2  EditableElement (新文字，可编辑)
                ↓ 用户看到的是这个
z-index: 1  text-mask (白色遮罩，opacity: 0.95)
                ↓ 遮蔽原文字
z-index: 0  原始图片 (包含不可编辑的原文字)
```

**遮罩效果示意**：

```
无遮罩（错误效果）:
┌────────────────────────────┐
│ 人工智能技术发展现状      │ ← 新文字（可编辑）
│ 人工智能技术发展现状      │ ← 原文字（不可编辑）
│ 重叠显示，看不清          │
└────────────────────────────┘

有遮罩（正确效果）:
┌────────────────────────────┐
│                          │ ← 白色遮罩
│ 人工智能技术发展现状      │ ← 新文字（可编辑）
│                          │
└────────────────────────────┘
```

**遮罩样式优化**：

```css
/* 方案1：纯白色遮罩（推荐） */
.text-mask {
  background: white;
  opacity: 0.95;
}

/* 方案2：根据背景色调整（如果原背景不是白色） */
.text-mask {
  background: var(--slide-background-color, white);
  opacity: 0.95;
}

/* 方案3：模糊遮罩（效果更柔和） */
.text-mask {
  background: white;
  opacity: 0.9;
  backdrop-filter: blur(2px);
}
```

#### 5.4.3 完整的数据流

```
┌────────────────┐
│ 1. 用户点击    │
│    "解析图片"  │
└───────┬────────┘
        │
        ▼
┌────────────────┐
│ 2. 前端调用    │ ← imageParsingService.parseSlideImage()
│    API         │
└───────┬────────┘
        │
        ▼
┌────────────────┐
│ 3. 后端 OCR    │ ← TencentOCRSDKEngine.parse_from_url()
│    识别        │
└───────┬────────┘
        │
        ▼ 返回 TextRegion[]
┌────────────────────────────────────┐
│ 4. 前端接收解析结果                │
│    {                              │
│      id: "region_001",            │
│      text: "人工智能技术",        │
│      bbox: {x, y, width, height}  │
│    }                              │
└───────┬────────────────────────────┘
        │
        ▼ toTextElement() 转换
┌────────────────────────────────────┐
│ 5. 转换为 PPTTextElement 格式      │
│    {                              │
│      type: "text",                │
│      content: "人工智能技术",     │
│      defaultFontSize: 42,         │
│      x: 0, y: 0, width, height    │
│    }                              │
└───────┬────────────────────────────┘
        │
        ▼
┌────────────────────────────────────┐
│ 6. EditableElement 渲染            │
│    (复用现有组件✅)                │
│    - 双击可编辑                    │
│    - 拖拽可移动                    │
│    - 支持所有文本编辑功能          │
└────────────────────────────────────┘
        │
        ▼ 用户编辑（仅内存）
```

---

## 6. 后端实现

### 6.1 腾讯云 OCR API 集成（MVP 首选）

#### 6.1.1 准备工作

**步骤1：注册腾讯云并开通OCR服务（5分钟）**

```bash
# 1. 注册腾讯云账号
https://cloud.tencent.com/

# 2. 开通OCR服务（自动开通，无需审核）
https://cloud.tencent.com/product/ocr

# 3. 获取API密钥
控制台 -> 访问密钥 -> API密钥 -> 新建密钥
# 记录 SecretId 和 SecretKey
```

**步骤2：配置环境变量**

```bash
# backend/.env
TENCENT_OCR_SECRET_ID=AKIDxxxxxxxxxxxxxxxxxxxxxxxxxxxx
TENCENT_OCR_SECRET_KEY=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
TENCENT_OCR_REGION=ap-guangzhou
```

#### 6.1.2 OCR 引擎实现（使用SDK）

腾讯云提供Python SDK，签名自动处理，无需手动实现复杂的签名算法：

```bash
pip install tencentcloud-sdk-python
```

```python
# backend/app/services/ocr/tencent_ocr_sdk_engine.py
from tencentcloud.common import credential
from tencentcloud.ocr.v20181119 import ocr_client, models
from typing import List, Dict
import os

class TencentOCRSDKEngine:
    """腾讯云OCR引擎 - SDK版本（更简单）"""

    def __init__(self):
        # 使用SDK，签名自动处理
        secret_id = os.getenv("TENCENT_OCR_SECRET_ID")
        secret_key = os.getenv("TENCENT_OCR_SECRET_KEY")
        region = os.getenv("TENCENT_OCR_REGION", "ap-guangzhou")

        cred = credential.Credential(secret_id, secret_key)
        self.client = ocr_client.OcrClient(cred, region)

    def parse_from_url(self, image_url: str) -> List[Dict]:
        """从URL解析图片"""
        # 下载图片
        import requests
        import base64
        img_data = requests.get(image_url).content
        img_base64 = base64.b64encode(img_data).decode()

        # 调用API（通用印刷体识别）
        req = models.GeneralBasicOCRRequest()
        req.ImageBase64 = img_base64

        resp = self.client.GeneralBasicOCR(req)

        # 格式化结果
        formatted = []
        for item in resp.TextDetections:
            polygon = item.Polygon
            if polygon:
                bbox = {
                    "x": int(min(p.X for p in polygon)),
                    "y": int(min(p.Y for p in polygon)),
                    "width": int(max(p.X for p in polygon) - min(p.X for p in polygon)),
                    "height": int(max(p.Y for p in polygon) - min(p.Y for p in polygon))
                }
            else:
                bbox = {
                    "x": item.ItemCoord.X,
                    "y": item.ItemCoord.Y,
                    "width": item.ItemCoord.Width,
                    "height": item.ItemCoord.Height
                }

            formatted.append({
                "text": item.DetectedText,
                "bbox": bbox,
                "confidence": item.Confidence / 100 if item.Confidence else 0.95
            })

        return formatted
```

#### 6.1.3 快速测试

```python
# test_ocr.py
import os
from app.services.ocr.tencent_ocr_sdk_engine import TencentOCRSDKEngine

# 测试
engine = TencentOCRSDKEngine()
result = engine.parse_from_url("https://xxx.com/test.png")

for item in result:
    print(f"文字: {item['text']}")
    print(f"位置: {item['bbox']}")
    print(f"置信度: {item['confidence']}")
    print("---")
```

#### 6.1.4 图片解析服务

```python
# backend/app/services/parsing/image_parsing_service.py
from typing import List, Dict, Optional
from datetime import datetime
import uuid
import asyncio
from sqlalchemy.ext.asyncio import AsyncSession

from app.services.ocr.tencent_ocr_sdk_engine import TencentOCRSDKEngine
from app.models.schemas.image_parsing import (
    TextRegion, ParseRequest, ParseResult, ParseMetadata
)
from app.repositories.image_parsing_repository import ImageParsingRepository


class ImageParsingService:
    """图片解析服务"""

    def __init__(self, db: AsyncSession):
        self.db = db
        self.repo = ImageParsingRepository(db)
        self.ocr_engine = None

    def _get_ocr_engine(self, options: Optional[Dict] = None) -> TencentOCRSDKEngine:
        """获取 OCR 引擎实例（MVP: 腾讯云OCR）"""
        if self.ocr_engine is None:
            self.ocr_engine = TencentOCRSDKEngine()
        return self.ocr_engine

    async def parse_image(
        self,
        slide_id: str,
        image_url: str,
        options: Optional[Dict] = None,
        user_id: Optional[str] = None
    ) -> ParseResult:
        """解析图片中的文字"""
        start_time = datetime.now()
        task_id = f"parse_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:8]}"

        # 创建任务记录
        await self.repo.create_task(
            task_id=task_id,
            slide_id=slide_id,
            image_url=image_url,
            user_id=user_id,
            status="processing"
        )

        try:
            # 获取 OCR 引擎
            ocr_engine = self._get_ocr_engine(options)

            # 执行 OCR 识别
            ocr_results = await asyncio.to_thread(
                ocr_engine.parse_from_url,
                image_url
            )

            # 构建文字区域数据
            text_regions = []
            for idx, result in enumerate(ocr_results):
                region_id = f"region_{idx + 1:03d}"

                text_region = TextRegion(
                    id=region_id,
                    text=result["text"],
                    bbox=result["bbox"],
                    confidence=result["confidence"],
                    font={
                        "size": self._infer_font_size(result["bbox"]),
                        "family": "Microsoft YaHei",
                        "weight": self._infer_font_weight(result)
                    }
                )
                text_regions.append(text_region)

            # 计算解析耗时
            parse_time = int((datetime.now() - start_time).total_seconds() * 1000)

            # 构建元数据
            metadata = ParseMetadata(
                parse_time=parse_time,
                ocr_engine="tencent_ocr",
                text_count=len(text_regions),
                created_at=start_time,
                completed_at=datetime.now()
            )

            # 更新任务状态
            await self.repo.update_task_status(
                task_id=task_id,
                status="completed",
                text_regions=[r.dict() for r in text_regions],
                metadata=metadata.dict()
            )

            return ParseResult(
                task_id=task_id,
                slide_id=slide_id,
                image_url=image_url,
                status="completed",
                progress=100,
                text_regions=text_regions,
                metadata=metadata
            )

        except Exception as e:
            await self.repo.update_task_status(
                task_id=task_id,
                status="failed",
                error_message=str(e)
            )
            raise

    def _infer_font_size(self, bbox: Dict) -> int:
        """推断字体大小"""
        estimated_size = int(bbox["height"] * 0.75)
        return max(12, min(72, estimated_size))

    def _infer_font_weight(self, result: Dict) -> str:
        """推断字重"""
        text = result.get("text", "")
        bbox = result.get("bbox", {})
        if len(text) < 20 and bbox.get("height", 0) > 40:
            return "bold"
        return "normal"
```

---

## 7. 数据库设计

### 7.1 解析任务表

```sql
-- 解析任务表
CREATE TABLE image_parse_tasks (
    id VARCHAR(50) PRIMARY KEY,                    -- 任务ID
    slide_id VARCHAR(100) NOT NULL,                -- 关联的幻灯片ID
    image_url TEXT NOT NULL,                       -- 原始图片URL
    status parse_task_status NOT NULL,             -- 任务状态
    progress INTEGER DEFAULT 0,                    -- 进度 0-100

    -- 解析选项
    parse_options JSONB,                           -- 解析选项配置

    -- 解析结果
    text_regions JSONB,                            -- 解析出的文字区域
    metadata JSONB,                                -- 解析元数据

    -- 时间戳
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    started_at TIMESTAMP WITH TIME ZONE,
    completed_at TIMESTAMP WITH TIME ZONE,

    -- 错误信息
    error_message TEXT,

    -- 用户关联
    user_id VARCHAR(50),

    -- 外键
    FOREIGN KEY (user_id) REFERENCES users(id)
);

-- 任务状态枚举
CREATE TYPE parse_task_status AS ENUM (
    'pending',      -- 等待处理
    'downloading',  -- 下载图片中
    'processing',   -- 解析处理中
    'completed',    -- 解析完成
    'failed'        -- 解析失败
);

-- 索引
CREATE INDEX idx_parse_tasks_slide_id ON image_parse_tasks(slide_id);
CREATE INDEX idx_parse_tasks_user_id ON image_parse_tasks(user_id);
CREATE INDEX idx_parse_tasks_status ON image_parse_tasks(status);
CREATE INDEX idx_parse_tasks_created_at ON image_parse_tasks(created_at DESC);

-- 注释
COMMENT ON TABLE image_parse_tasks IS '图片解析任务表';
COMMENT ON COLUMN image_parse_tasks.text_regions IS '解析出的文字区域，包含文字内容、位置、置信度等信息';
```

### 7.2 Redis 缓存设计

```python
# 任务状态缓存（TTL: 1小时）
Key: image:parse:task:{task_id}
Value: {
    "task_id": "parse_20241224_123456_abc123",
    "status": "processing",
    "progress": 65,
    "current_step": "ocr_recognition",
    "slide_id": "banana_xxx_slide_0",
    "created_at": "2024-12-24T12:34:56Z"
}

# 解析结果缓存（TTL: 24小时）
Key: image:parse:result:{task_id}
Value: {
    "task_id": "parse_20241224_123456_abc123",
    "slide_id": "banana_xxx_slide_0",
    "image_url": "https://xxx.cos.ap-beijing.myqcloud.com/...",
    "status": "completed",
    "progress": 100,
    "text_regions": [
        {
            "id": "region_001",
            "text": "人工智能技术发展现状",
            "bbox": {"x": 250, "y": 80, "width": 500, "height": 60},
            "confidence": 0.98,
            "font": {"size": 42, "family": "Microsoft YaHei", "weight": "bold"}
        },
        ...
    ],
    "metadata": {
        "parse_time": 3200,
        "ocr_engine": "paddleocr",
        "text_count": 3,
        "created_at": "2024-12-24T12:34:56Z",
        "completed_at": "2024-12-24T12:35:00Z"
    }
}

# 幻灯片解析状态汇总（TTL: 1小时）
Key: image:parse:slides:{slide_id}
Value: {
    "slide_id": "banana_xxx_slide_0",
    "is_parsed": true,
    "text_count": 3,
    "task_id": "parse_20241224_123456_abc123",
    "parsed_at": "2024-12-24T12:35:00Z"
}
```

---

## 8. API 端点实现

### 8.1 端点定义

```python
# backend/app/api/v1/endpoints/image_parsing.py
from fastapi import APIRouter, Depends, HTTPException
from sqlalchemy.ext.asyncio import AsyncSession

from app.api.dependencies import get_db, get_current_user_id
from app.models.schemas.image_parsing import (
    ParseRequest, ParseTaskResponse, ParseStatusResponse
)
from app.services.parsing.image_parsing_service import ImageParsingService
from app.core.standard_response import StandardResponse

router = APIRouter()


@router.post("/parse", response_model=StandardResponse[ParseTaskResponse])
async def parse_image(
    request: ParseRequest,
    db: AsyncSession = Depends(get_db),
    user_id: str = Depends(get_current_user_id)
):
    """
    解析图片中的文字

    提交图片解析任务，返回任务ID。前端需要轮询查询任务状态获取解析结果。
    """
    service = ImageParsingService(db)

    # 提交到后台任务队列（Celery）
    from app.tasks.parsing_tasks import parse_image_task
    task = parse_image_task.apply_async(
        kwargs={
            "slide_id": request.slide_id,
            "image_url": request.image_url,
            "options": request.options.dict() if request.options else None,
            "user_id": user_id
        },
        queue="image_parsing"
    )

    # 创建任务记录
    task_id = f"parse_{task.id}"

    return StandardResponse.success(
        data=ParseTaskResponse(
            task_id=task_id,
            status="pending",
            estimated_time=5
        ),
        message="解析任务已创建"
    )


@router.get("/status/{task_id}", response_model=StandardResponse[ParseStatusResponse])
async def get_parse_status(
    task_id: str,
    db: AsyncSession = Depends(get_db)
):
    """
    查询解析状态

    返回任务的当前状态和解析结果（如果已完成）。
    """
    service = ImageParsingService(db)
    result = await service.get_parse_result(task_id)

    if not result:
        raise HTTPException(status_code=404, detail="任务不存在")

    return StandardResponse.success(data=result)
```

### 8.2 Celery 任务

```python
# backend/app/tasks/parsing_tasks.py
from celery import shared_task
from sqlalchemy.ext.asyncio import create_async_engine, AsyncSession
from app.core.config import settings
from app.services.parsing.image_parsing_service import ImageParsingService


@shared_task(queue="image_parsing", bind=True)
def parse_image_task(
    self,
    slide_id: str,
    image_url: str,
    options: dict = None,
    user_id: str = None
):
    """
    异步图片解析任务
    """
    import asyncio

    async def _parse():
        engine = create_async_engine(settings.database_url)
        async with AsyncSession(engine) as db:
            service = ImageParsingService(db)
            result = await service.parse_image(
                slide_id=slide_id,
                image_url=image_url,
                options=options,
                user_id=user_id
            )
            return result

    # 运行异步任务
    result = asyncio.run(_parse())
    return result.dict()
```

---

## 9. 前端 API 配置

### 9.1 API 端点配置（修改现有文件）

**文件**：`frontend/src/configs/api.ts`

在现有的 `API_CONFIG` 中添加 `IMAGE_PARSING` 配置：

```typescript
// frontend/src/configs/api.ts
export const API_CONFIG = {
  // ... 现有配置 (IMAGES, IMAGE_PROXY, IMAGE_UPLOAD, TAGS, etc.)

  // 🔧 新增：图片解析相关API
  IMAGE_PARSING: {
    // 解析图片中的文字
    PARSE: '/api/v1/image_parsing/parse',
    // 查询解析状态
    STATUS: (taskId: string) => `/api/v1/image_parsing/status/${taskId}`,
  },

  // ... 其他现有配置
} as const

export default API_CONFIG
```

### 9.2 服务封装（新建文件）

**文件**：`frontend/src/services/imageParsingService.ts`（新建）

```typescript
// frontend/src/services/imageParsingService.ts
import http from '@/utils/request'
import { API_CONFIG } from '@/configs/api'
import type {
  ImageParseResult,
  ParseTaskResponse,
  ParseStatusResponse
} from '@/types/imageParsing'

/**
 * 图片解析服务
 */
export const imageParsingService = {
  /**
   * 解析图片中的文字
   * @param slideId 幻灯片ID
   * @param imageUrl 图片URL
   * @returns 解析任务响应
   */
  async parseSlideImage(
    slideId: string,
    imageUrl: string
  ): Promise<ParseTaskResponse> {
    const { data } = await http.post(API_CONFIG.IMAGE_PARSING.PARSE, {
      slide_id: slideId,
      image_url: imageUrl
    })
    return data
  },

  /**
   * 查询解析状态
   * @param taskId 任务ID
   * @returns 解析结果
   */
  async getParsingStatus(taskId: string): Promise<ImageParseResult> {
    const { data } = await http.get(
      API_CONFIG.IMAGE_PARSING.STATUS(taskId)
    )
    return data
  }
}

export default imageParsingService
```

**说明**：
- 复用现有的 `http` 客户端（来自 `@/utils/request`）
- 遵循项目现有的服务层封装模式
- 参考 `frontend/src/services/index.ts` 的风格

### 9.3 类型定义（新建文件）

**文件**：`frontend/src/types/imageParsing.ts`（新建）

```typescript
// frontend/src/types/imageParsing.ts

/**
 * 解析任务响应
 */
export interface ParseTaskResponse {
  task_id: string
  status: 'pending' | 'processing' | 'completed' | 'failed'
  estimated_time: number
  message?: string
}

/**
 * 解析状态响应（完成时包含完整结果）
 */
export interface ParseStatusResponse extends ImageParseResult {}

/**
 * 边界框坐标
 */
export interface BoundingBox {
  x: number
  y: number
  width: number
  height: number
}

/**
 * 字体信息
 */
export interface FontInfo {
  size: number
  family: string
  weight: 'normal' | 'bold'
  color?: string
}

/**
 * 文字区域
 */
export interface TextRegion {
  id: string
  text: string
  bbox: BoundingBox
  confidence: number
  font?: FontInfo
  // 前端编辑时临时使用（不持久化，刷新后重置）
  _isModified?: boolean
  _originalText?: string
}

/**
 * 解析元数据
 */
export interface ParseMetadata {
  parse_time: number
  ocr_engine: string
  text_count: number
  created_at: string
  completed_at?: string
}

/**
 * 图片解析结果
 */
export interface ImageParseResult {
  task_id: string
  slide_id: string
  image_url: string
  status: 'pending' | 'processing' | 'completed' | 'failed'
  progress: number
  text_regions: TextRegion[]
  metadata: ParseMetadata
  message?: string
}
```

**说明**：
- 遵循项目现有的类型定义风格（参考 `types/slides.ts`）
- 所有类型都添加了 JSDoc 注释
- 支持现有的 TypeScript 类型检查

---

## 10. 实施路线图

### 10.1 分阶段实施计划

#### 第一阶段：核心功能实现 (1-2周)

**目标**: 实现基础的文字识别和文字框显示

- [ ] 准备工作：
  - [ ] 注册腾讯云账号并开通OCR服务（5分钟）
  - [ ] 获取API密钥并配置环境变量

- [ ] 后端：
  - [ ] 搭建 API 框架和数据库表结构
  - [ ] 集成腾讯云OCR SDK（10分钟）
  - [ ] 实现图片解析服务
  - [ ] 实现异步任务处理（Celery）

- [ ] 前端：
  - [ ] 添加解析按钮到工具条
  - [ ] 实现图片叠加层组件
  - [ ] 实现文字框显示组件
  - [ ] 实现轮询获取解析结果

**交付标准**:
- 用户可以点击解析按钮
- 能识别图片中的主要文字（95%+准确率）
- 在图片上显示文字框位置
- 文字框显示识别的内容
- 演示效果良好

#### 第二阶段：交互功能 (1周)

**目标**: 实现文字框的编辑和拖拽功能

- [ ] 前端：
  - [ ] 实现文字框点击编辑功能
  - [ ] 实现文字框拖拽移动
  - [ ] 实现文字框拖拽调整大小

**交付标准**:
- 双击文字框可以编辑文字
- 拖拽文字框可以移动位置
- 拖拽边角可以调整大小

**注意**：本阶段不涉及后端保存，编辑内容仅存在于前端内存中

#### 第三阶段：优化与完善 (1周)

**目标**: 优化性能，提升用户体验

- [ ] 性能优化：
  - [ ] 添加 Redis 缓存
  - [ ] 优化前端渲染性能
  - [ ] 实现 WebSocket 实时推送（可选）

- [ ] 用户体验：
  - [ ] 添加加载动画和进度提示
  - [ ] 添加错误处理和重试机制
  - [ ] 优化文字框样式

**交付标准**:
- 解析速度 < 5 秒
- 流畅的编辑体验
- 完善的错误处理

### 10.2 技术里程碑

| 阶段 | 里程碑 | 完成标准 |
|------|--------|----------|
| Phase 1 | MVP发布 | 文字可识别和显示 |
| Phase 2 | 可编辑 | 文字可编辑和移动 |
| Phase 3 | 生产就绪 | 性能和稳定性达标 |

---

## 11. 风险与挑战

### 11.1 技术风险

| 风险 | 影响 | 概率 | 缓解措施 |
|------|------|------|----------|
| OCR识别准确率不足 | 高 | 低 | 1. MVP阶段使用腾讯云OCR（95%+准确率）<br>2. 未来可切换到更高精度的方案<br>3. 添加置信度提示<br>4. 支持用户手动修正编辑 |
| 性能瓶颈 | 中 | 低 | 1. MVP阶段CPU即可<br>2. 生产环境使用GPU加速<br>3. 异步处理<br>4. Redis缓存结果 |
| 服务器资源占用 | 中 | 中 | 1. 使用Celery任务队列<br>2. 限制并发数量<br>3. 监控资源使用 |

### 11.2 业务风险

| 风险 | 影响 | 概率 | 缓解措施 |
|------|------|------|----------|
| 用户期望过高 | 中 | 高 | 1. 明确功能边界<br>2. 提供使用说明<br>3. 收集反馈持续优化 |

---

## 12. 附录

### 12.1 相关资料链接

**OCR与文档解析（MVP 阶段推荐）**:
- [腾讯云 OCR API](https://cloud.tencent.com/product/ocr) - 首选方案，95%+准确率
- [腾讯云 Python SDK](https://cloud.tencent.com/document/sdk/Python)
- [阿里云 OCR API](https://www.aliyun.com/product/ocr) - 备选方案

**生产环境备选方案**:
- [PaddleOCR 官方文档](https://paddlepaddle.github.io/PaddleOCR/main/en/index.html)
- [PaddleOCR GitHub](https://github.com/PaddlePaddle/PaddleOCR)

**前端技术**:
- [Vue 3 官方文档](https://vuejs.org/)
- [TypeScript 官方文档](https://www.typescriptlang.org/)

### 12.2 术语表

| 术语 | 说明 |
|------|------|
| OCR | Optical Character Recognition，光学字符识别 |
| 文字区域 | 图片中检测到的文字区域，包含文字内容和位置信息 |
| 边界框 (BBox) | Bounding Box，包围文字区域的矩形框 |
| 置信度 | OCR识别结果的可信程度，0-1之间的值 |
| 文生图 | Text-to-Image Generation，根据文字描述生成图片 |
| 香蕉生成 | 本项目的PPT生成功能代号 |

### 12.3 版本历史

| 版本 | 日期 | 作者 | 说明 |
|------|------|------|------|
| v1.0 | 2024-12-24 | Claude Code | 初始版本 |
| v2.0 | 2024-12-24 | Claude Code | 简化版本，移除PPTX导出，聚焦文字识别 |
| v3.0 | 2024-12-24 | Claude Code | MVP优化，推荐EasyOCR作为快速原型方案 |
| v4.0 | 2024-12-24 | Claude Code | MVP优化，推荐腾讯云OCR API（最佳演示效果） |
| v5.0 | 2024-12-24 | Claude Code | **范围澄清**：移除保存功能，明确编辑内容仅存在于前端内存 |
| v6.0 | 2024-12-24 | Claude Code | **前端方案优化**：复用 EditableElement + 遮罩层替代自定义 DraggableTextBox |

---

## 参考资料

**MVP 阶段（腾讯云OCR）**:
1. [腾讯云 OCR 产品](https://cloud.tencent.com/product/ocr)
2. [腾讯云 Python SDK](https://cloud.tencent.com/document/sdk/Python)
3. [腾讯云 OCR API 文档 - 通用印刷体识别](https://cloud.tencent.com/document/product/866/33526)

**生产环境备选**:
4. [PaddleOCR Documentation](https://paddlepaddle.github.io/PaddleOCR/)
5. [PaddleOCR GitHub](https://github.com/PaddlePaddle/PaddleOCR)
6. [阿里云 OCR](https://www.aliyun.com/product/ocr)

**后端框架**:
7. [FastAPI Documentation](https://fastapi.tiangolo.com/)
8. [Celery Documentation](https://docs.celeryq.dev/)

**前端框架**:
9. [Vue 3 Documentation](https://vuejs.org/)
10. [TypeScript Documentation](https://www.typescriptlang.org/)
